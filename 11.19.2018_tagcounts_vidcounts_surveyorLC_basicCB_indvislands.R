...
#Create one population estimation model which combines
#tag count data from Little Cayman (pop 1)
#tag count data from Cayman Brac (pop 2)
#video ccounts from Little Cayman (pop 1)
#video counts from Cayman Brac (pop 2)
#estimate population size from start of earlies time series on each island
#share proportion parameter for fraction of "TRUE" abundance seen on video
#allow unique growth rate per island
#include observer effect on Little Cayman for tag counts where we know it matters
...
cat("
    ...
    for (nn in 1:length(lvid.number)){
    lvid.number[nn]~dnorm( (log(vid.prop)+N.est[vid.year[nn]-(yr.vid.startLC-1)]),
...
jags.data.tagvid.Lconly <- list(TAG_CNT=TAG_CNT,TAG_NUM=TAG_NUM,Year=Year,
                                ...
                                vid.year=vid.LC.year,lvid.number=log(vid.LC.number),
...
library(mcmcplots)
mcmcplot(ssm_grouper_tagvid.Lconly)
...
num.Person<-length(unique(Person))
library(R2jags)
sink("ssm.tags.jags.tagvid.Lconly_yrP")
cat("
    model { 
    # Priors and constraints for LC
    N.est[1] ~ dunif(log(1),log(5000))
    ...
    #hierarchical mean - priors to video params estimated from LC 
    vid.prop.mean~dunif(0,1) #overall mean proporation
    sigma.vidp ~ dunif(0, 100)           # Prior for sd of video prop
    sigma2.vidp <- pow(sigma.vidp, 2)
    tau.vidp <- pow(sigma.vidp, -2)
    logit.v.probs<-log(vid.prop.mean/(1-vid.prop.mean)) #overall mean prop in logit space
    
    for(no in uni.vid.year){
    dumdum.v[no]~dnorm(logit.v.probs,tau.vidp)
    vid.prop[no]<-exp(dumdum.v[no])/(1+exp(dumdum.v[no])) #turn into logistic- yearly proportion
    }
    ...
    #match video observations on LC
    for (nn in 1:length(lvid.number)){
    lvid.number[nn]~dnorm( (log(vid.prop[vid.year[nn]-(yr.vid.startLC-1)])+exp(N.est[vid.year[nn]-(yr.vid.startLC-1)])),
    tau.vid[vid.year[nn]-(yr.vid.startLC-1)])
    }
...
# build data, parameters and MCMC settings for JAGS:
###################################################################
jags.data.tagvid.Lconly_yrP <- list(TAG_CNT=TAG_CNT,TAG_NUM=TAG_NUM,Year=Year,
                                SURVEY_NUM=SURVEY_NUM,Person=Person, 
                                Person.no=unique(Person),
                                vid.year=vid.LC.year,lvid.number=log(vid.LC.number),
                                uni.vid.year=unique(vid.LC.year)-(yr.vid.startLC-1),
...
# What should JAGS keep track of
jags.params.tagvid.Lconly_yrP = c("N.est","mu","mean.mu","sigma2.proc","percep.sigma2","percep",
                              "vid.prop","sigma2.vid","vid.prop.mean")

#MCMC settings
mcmc.chainLength <- as.integer(400000)  #Chainlength 100,000
mcmc.burn <- as.integer (40000)         #number samples to burn 40,000
mcmc.thin = 480                       #thinning interval 120
mcmc.chains = 16                        #number of MCMC chains 4

############################### run the model in JAGS########################################
ssm_grouper_tagvid.Lconly_yrP<- jags(jags.data.tagvid.Lconly_yrP, inits=NULL, 
                                 parameters.to.save= jags.params.tagvid.Lconly_yrP, 
                                 "ssm.tags.jags.tagvid.Lconly_yrP", n.chains = mcmc.chains, 
                                 n.thin = mcmc.thin, n.iter = mcmc.chainLength, 
                                 n.burnin = mcmc.burn, working.directory = getwd(),
                                 DIC=TRUE)
attach.jags(ssm_grouper_tagvid.Lconly_yrP)
ssm_grouper_tagvid.Lconly_yrP
DIC_model_surveyor<-ssm_grouper_tagvid.Lconly_yrP$BUGSoutput$DIC

save.image(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),"Model.tagvid.Lconly_yrP", "RData", sep = "."))
library(mcmcplots)
mcmcplot(ssm_grouper_tagvid.Lconly_yrP)
...
a<-func.gelman(ssm_grouper_tagvid.Lconly_yrP)
a

sink(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),
           "gelmanrubin_ssm_grouper.tagvid.Lconly_yrP.csv",sep=""))
print(a)
sink()
##########################################################################################
##########################################################################################
...
pm.Year<-c(rep(seq(2005,2018,by=1),each=56000/14))
...
mcmc.chainLength <- as.integer(400000)  #Chainlength 100,000
mcmc.burn <- as.integer (40000)         #number samples to burn 40,000
mcmc.thin = 400                      #thinning interval 120
mcmc.chains = 16                        #number of MCMC chains 4
...
#run state space model
library(R2jags)
sink("ssm.tags.jags.tagvid.CBonly.yrp")
cat("
    model { 
    # Priors and constraints for LC - just basic model
    N.est.CB[1] ~ dunif(log(1),log(5000))
    ...
    #hierarchical mean - priors to video params estimated from LC 
    vid.prop.mean.CB~dunif(0,1) #overall mean proporation
    sigma.vidp.CB ~ dunif(0, 100)           # Prior for sd of video prop
    sigma2.vidp.CB <- pow(sigma.vidp.CB, 2)
    tau.vidp.CB <- pow(sigma.vidp.CB, -2)
    logit.v.probs.CB<-log(vid.prop.mean.CB/(1-vid.prop.mean.CB)) #overall mean prop in logit space
    
    for(no in uni.vid.year.CB){
    dumdum.CB.v[no]~dnorm(logit.v.probs.CB,tau.vidp.CB)
    vid.prop.CB[no]<-exp(dumdum.CB.v[no])/(1+exp(dumdum.CB.v[no])) #turn into logistic- yearly proportion
    }
    ...
    #match video observations on CB
    for (nnn in 1:length(vid.number.CB)){
    vid.number.CB[nnn]~dnorm(vid.prop.CB[vid.year.CB[nnn]-(yr.vid.startCB-1)]*exp(N.est.CB[vid.year.CB[nnn]-(yr.vid.startCB-1)]),
    tau.vid.CB[vid.year.CB[nnn]-(yr.vid.startCB-1)])
    }
...
# build data, parameters and MCMC settings for JAGS:
###################################################################
jags.data.tagvid.CBonly.yrp <- list(#CB parameters
  TAG_CNT.CB=TAG_CNT.CB,TAG_NUM.CB=TAG_NUM.CB,Year.CB=Year.CB,
  SURVEY_NUM.CB=SURVEY_NUM.CB,
  vid.year.CB=vid.CB.year,vid.number.CB=vid.CB.number,
  uni.vid.year.CB=unique(vid.CB.year)-(yr.vid.startCB-1),
...
# What should JAGS keep track of
jags.params.tagvid.CBonly.yrp = c("N.est.CB","mu.CB","mean.mu.CB","sigma2.proc.CB",
                                   "sigma2.vid.CB","vid.prop.CB","vid.prop.mean.CB")

#MCMC settings
mcmc.chainLength <- as.integer(4000000)  #Chainlength 100,000
mcmc.burn <- as.integer (1600000)         #number samples to burn 40,000
mcmc.thin = 480                      #thinning interval 120
mcmc.chains = 16                        #number of MCMC chains 4

############################### run the model in JAGS########################################
ssm_grouper_tagvid.CBonly.yrp<- jags(jags.data.tagvid.CBonly.yrp, inits=NULL, 
                                      parameters.to.save= jags.params.tagvid.CBonly.yrp, 
                                      "ssm.tags.jags.tagvid.CBonly.yrp", n.chains = mcmc.chains, 
                                      n.thin = mcmc.thin, n.iter = mcmc.chainLength, 
                                      n.burnin = mcmc.burn, working.directory = getwd(),
                                      DIC=TRUE)
attach.jags(ssm_grouper_tagvid.CBonly.yrp)
ssm_grouper_tagvid.CBonly.yrp

save.image(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),"Model.tagvid.CBonly.yrp",
                 "RData", sep = "."))
...
a<-func.gelman(ssm_grouper_tagvid.CBonly.yrp)
a

sink(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),
           "gelmanrubin_ssm_grouper.tagvid.CBonly.yrp.csv",sep=""))
print(a)
sink()
...
cat("
    ...
    for (ii in 1:length(TAG_CNT.CB)){
    TAG_CNT.CB[ii] ~ dbin(probs.CB[ii], SURVEY_NUM.CB[ii]) #tag counts come from binomial distribution
    probs.CB[ii]<-TAG_NUM.CB[ii]/(exp(N.est.CB[Year.CB[ii]-(start.CB-1)])*2) #proportion tagged, based on number tags out and sides of fish that year
    }
...
# build data, parameters and MCMC settings for JAGS:
###################################################################
jags.data.tag.CBonly <- list(#CB parameters
  TAG_CNT.CB=TAG_CNT.CB,TAG_NUM.CB=TAG_NUM.CB,Year.CB=Year.CB,
  SURVEY_NUM.CB=SURVEY_NUM.CB,start.CB=min(Year.CB),end.CB=max(Year.CB))

# What should JAGS keep track of
jags.params.tag.CBonly = c("N.est.CB","mu.CB","mean.mu.CB","sigma2.proc.CB")

#MCMC settings
mcmc.chainLength <- as.integer(1000000)  #Chainlength 100,000
mcmc.burn <- as.integer (40000)         #number samples to burn 40,000
mcmc.thin = 120                      #thinning interval 120
mcmc.chains = 4                        #number of MCMC chains 4

############################### run the model in JAGS########################################
ssm_grouper_tag.CBonly<- jags(jags.data.tag.CBonly, inits=NULL, 
                                     parameters.to.save= jags.params.tag.CBonly, 
                                     "ssm.tags.jags.tag.CBonly", n.chains = mcmc.chains, 
                                     n.thin = mcmc.thin, n.iter = mcmc.chainLength, 
                                     n.burnin = mcmc.burn, working.directory = getwd(),
                                     DIC=TRUE)
attach.jags(ssm_grouper_tag.CBonly)
ssm_grouper_tag.CBonly

save.image(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),"Model.tag.CBonly",
                 "RData", sep = "."))
...
a<-func.gelman(ssm_grouper_tag.CBonly)
a

sink(paste(format(Sys.time(), "%Y-%m-%d %I-%p"),
           "gelmanrubin_ssm_grouper.tag.CBonly.csv",sep=""))
print(a)
sink()
